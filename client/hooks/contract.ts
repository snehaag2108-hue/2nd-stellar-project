...
// ============================================================
// Token Burn — Contract Methods
// ============================================================

/**
 * Burns `amount` of `tokenAddr` tokens from `caller`.
 * Calls: burn(caller: Address, token_addr: Address, amount: i128)
 * Permissionless — anyone can burn their own tokens.
 */
export async function burnTokens(
  ...
  return callContract(
    "burn",
    [toScValAddress(caller), toScValAddress(tokenAddr), toScValI128(amount)],
    caller,
    true
  );
}
...
/**
 * Returns total tokens burned for a given token contract (read-only).
 * Calls: get_total_burned(token_addr: Address) -> i128
 */
...
/**
 * Returns how many tokens a specific user has burned for a token (read-only).
 * Calls: get_user_burned(token_addr: Address, user: Address) -> i128
 */
export async function getUserBurned(
  tokenAddr: string,
  user: string,
  caller?: string
): Promise<bigint> {
  const result = await readContract(
    "get_user_burned",
    [toScValAddress(tokenAddr), toScValAddress(user)],
    caller
  );
  return BigInt(result ?? 0);
}
...
/**
 * Returns the number of burn events recorded for a given token (read-only).
 * Calls: get_burn_count(token_addr: Address) -> u32
 */
export async function getBurnCount(
  tokenAddr: string,
  caller?: string
): Promise<number> {
  const result = await readContract(
